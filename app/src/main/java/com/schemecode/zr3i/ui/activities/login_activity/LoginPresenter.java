...
import com.schemecode.zr3i.data.models.CsrfToken;
...
public class LoginPresenter implements LoginContract.presenter {
    ...
    @Override
    public void getCrsToken() {
        Call<CsrfToken> loginCall = apiService.getCsrfToken();
        loginCall.enqueue(new Callback<CsrfToken>() {
            @Override
            public void onResponse(Call<CsrfToken> call, Response<CsrfToken> response) {
                Log.d("TAG", "saveCsrfToken: "+response.body().getCsrf_token());

                if (response.isSuccessful()){
                  view.saveCsrfToken(response.body().getCsrf_token());
                }
            }
