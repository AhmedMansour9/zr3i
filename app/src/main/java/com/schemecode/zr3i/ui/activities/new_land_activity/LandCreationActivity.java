...
public class LandCreationActivity extends AppCompatActivity implements OnMapReadyCallback, LandCreationContract.view {
    ...
    public void getListOfCountires() {
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (owner.matches("") || space.matches("")
                    ...
                    StoreLandRequest storeLandRequest = new StoreLandRequest();
                    storeLandRequest.setActualSpace(Double.parseDouble(space));
                    storeLandRequest.setCity(city);
                    storeLandRequest.setCityArea(hay);
                    storeLandRequest.setContractSpace(Integer.parseInt(contractSpace));
                    storeLandRequest.setCountryId(country);
                    storeLandRequest.setCustomerId(1);
                    storeLandRequest.setCropId(cropId);
                    storeLandRequest.setCropPlantingCycles(numberOfCropCycles);
                    storeLandRequest.setOwner(owner);
                    com.schemecode.zr3i.data.models.store_land.Map googleMap= new com.schemecode.zr3i.data.models.store_land.Map();
                    googleMap.setLatitude(currentMapCordinator.latitude);
                    googleMap.setLongitude(currentMapCordinator.longitude);
                    googleMap.setZoom(zoom);
                    googleMap.setPolygon(latLngListToSent);
                    CropField cropField=new CropField();
                    cropField.setCropId(cropId);
                    cropField.setMap(googleMap);
                    List<CropField> cropFields = new ArrayList<>();
                    cropFields.add(cropField);
                    storeLandRequest.setCropFields(cropFields);



                    landCreationPresenter.storeLand(storeLandRequest);
                }
            }
        });
    }
