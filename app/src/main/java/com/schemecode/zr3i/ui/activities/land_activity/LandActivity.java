...
import com.schemecode.zr3i.adapters.WeatherAdapter;
import com.schemecode.zr3i.adapters.WeatherExpectationsAdapter;
import com.schemecode.zr3i.adapters.WeatherViewHolder;
...
public class LandActivity extends AppCompatActivity implements LandContract.view {
    ...
    private GridLayoutManager weatherExpectationLayoutManage;
...
//        weatherExpectationLayoutManage = new LinearLayoutManager(LandActivity.this, LinearLayoutManager.VERTICAL, false);
    ...
    List<ByDayTimeResponse> weatherDataList = new ArrayList<>();
...
//                }
                ...
                if (data.getSoilData().getT0() != null){
                    String tempt02f =   String.format("%.2f", data.getSoilData().getT0() - 273.15);;
                    String tempt102f =   String.format("%.2f", data.getSoilData().getT10() - 273.15);;
                    String temp10cm = "<font color='#72c02c'>"
                            + tempt102f + "</font>";
                    String tempInfo = "<font color='#72c02c'>"
                            + tempt02f + "</font>";
                    String moisture = "<font color='#72c02c'>"
                            + data.getSoilData().getMoisture() + "</font>";
//                    String uvi = "<font color='#72c02c'>"
//                            + data.getSoilData().getUvi().getUvi() + " " + data.getSoilData().getUvi().getRisk() + "</font>";
                    tv10CmTemp.setText(Html.fromHtml(temp10cm + " " + "\u2103"));
                    tvCurrentTemp.setText(Html.fromHtml(tempInfo + " " + "\u2103"));
...
//                    tvViolet.setText((Html.fromHtml(uvi)));
                ...
                if (data.getWeather() != null) {
                    String temp =  data.getWeather().getCurrent().getMain().getTemp()+"";
                    String humandity =  data.getWeather().getCurrent().getMain().getHumidity() + "";
                    String pressure =  data.getWeather().getCurrent().getMain().getPressure() + "";
                    String windSpeed =  data.getWeather().getCurrent().getWind().getDeg() + "," + data.getWeather().getCurrent().getWind().getSpeed() + " m/s (deg) " ;
...
//                    try {
//                        JSONArray byDayTimesArray = jsonResponse.getJSONArray("by_day_times");
//
//                        for (int i = 0; i < byDayTimesArray.length(); i++) {
//                            JSONObject jsonObject = byDayTimesArray.getJSONObject(i);
//                            Gson gson = new Gson();
//                            List<WeatherResponse> weatherResponses = new ArrayList<>();
//
//
//                            Type listType = new TypeToken<List<ByDayTimeResponse>>() {}.getType();
//
//                            List<ByDayTimeResponse> itemList = gson.fromJson(jsonArrayString, listType);
//
//
////                            WeatherData weatherData = new WeatherData();
////                            weatherData.setDt(jsonObject.getLong("dt"));
////                            weatherData.setTemp(jsonObject.getJSONObject("temp").getDouble("day"));
////                            weatherData.setSummary(jsonObject.getString("summary"));
//
